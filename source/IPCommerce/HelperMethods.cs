...
using Response = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.Response;
using BankcardTransactionResponse = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardTransactionResponse;
using BankcardCaptureResponse = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardCaptureResponse;
using Capture = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.Capture;
using BankcardTransactionPro = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardTransactionPro;
using BankcardTransactionResponsePro = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardTransactionResponsePro;
using Addendum = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.Addendum;
using BankcardTransaction = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardTransaction;
using ElectronicCheckingCaptureResponse = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.ElectronicCheckingCaptureResponse;
using ElectronicCheckingTransaction = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.ElectronicCheckingTransaction;
using ElectronicCheckingTransactionResponse = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.ElectronicCheckingTransactionResponse;
using TypeCardType = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.TypeCardType;
using Unmanaged = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.Unmanaged;
using Status = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.Status;
using StoredValueTransaction = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.StoredValueTransaction;
...
namespace IPCommerce
{
    public class HelperMethods
    {
        ...
        public string BaseSvcEndpointPrimary =  ConfigurationManager.AppSettings["BaseSvcEndpointPrimary"]; //The following would likely come from a config value or database
        public string BaseSvcEndpointSecondary = ConfigurationManager.AppSettings["BaseSvcEndpointSecondary"]; //The following would likely come from a config value or database
        public string BaseTxnEndpointPrimary = ConfigurationManager.AppSettings["BaseTxnEndpointPrimary"]; //The following would likely come from a config value or database
        public string BaseTxnEndpointSecondary = ConfigurationManager.AppSettings["BaseTxnEndpointSecondary"]; //The following would likely come from a config value or database
        public string BaseTMSEndpointPrimary = ConfigurationManager.AppSettings["BaseDataServicesEndpointPrimary"]; //The following would likely come from a config value or database
        public string BaseTMSEndpointSecondary = ConfigurationManager.AppSettings["BaseDataServicesEndpointSecondary"]; //The following would likely come from a config value or database
        ...
        public bool CredentialRequired()
        {
            ...
            if (ServiceID == "8CEA11300C") return true; //Production
			if (ServiceID == "2C36000001") return true; //Sandbox
            if (ServiceID == "86FA11300C") return true; //Production
        ...
        {
            //Sandbox
            if (bankcardService.ServiceId == "214DF00001") return "Chase Paymentech Orbital - Tampa";
            if (bankcardService.ServiceId == "B51E100001") return "Chase Paymentech Orbital - Salem";
            if (bankcardService.ServiceId == "7B62B00001") return "First Data - Nashville";
            if (bankcardService.ServiceId == "786F400001") return "Chase Paymentech Orbital - Tampa Retail";
            if (bankcardService.ServiceId == "A656D00001") return "First Data - Nashville US";
            if (bankcardService.ServiceId == "4CACF00001") return "Chase Tampa Direct TCS";
            if (bankcardService.ServiceId == "3E2DE00001") return "RBS Global Gateway";
            if (bankcardService.ServiceId == "832E400001") return "RBS Worldpay";
            if (bankcardService.ServiceId == "C82ED00001") return "TSYS Sierra";
            if (bankcardService.ServiceId == "5A38100001") return "Tampa - Canada";
            if (bankcardService.ServiceId == "71C8700001") return "TSYS Sierra Canada";
            if (bankcardService.ServiceId == "8335000001") return "TSYS Summit";
            if (bankcardService.ServiceId == "A4F2B00001") return "Salem Direct";
            if (bankcardService.ServiceId == "E4FB800001") return "First Data - Nashville";
            if (bankcardService.ServiceId == "16E5800001") return "Intuit QBMS";
            if (bankcardService.ServiceId == "A8CFF00001") return "First Data BUYPASS";
            if (bankcardService.ServiceId == "36EBE00001") return "Tampa TCS for Canada";
            if (bankcardService.ServiceId == "6429C00001") return "Intuit QBMS Inline Tokenization";
            if (bankcardService.ServiceId == "8046100001") return "Intuit QBMS No Tokenization";
            if (bankcardService.ServiceId == "207CE00001") return "Adaptive Payments";
            if (bankcardService.ServiceId == "88D9300001") return "Vantiv FTPS";
            if (bankcardService.ServiceId == "8077500001") return "Intuit QBMS Inline Tokenization";
            if (bankcardService.ServiceId == "B447F00001") return "Vantiv IBM FTPS";
            if (bankcardService.ServiceId == "D806000001") return "Vantiv IBM FTPS";
            if (bankcardService.ServiceId == "9B0C200001") return "Intuit QBMS No Tokenization";
            if (bankcardService.ServiceId == "4365400001") return "Vantiv Tandem HC FTPS";
            if (bankcardService.ServiceId == "2C36000001") return "RBS Worldpay";
            if (bankcardService.ServiceId == "E88FD00001") return "TSYS Summit";
            if (bankcardService.ServiceId == "F061C00001") return "First Data - Nashville";

            //Production
            if (bankcardService.ServiceId == "C97EF1300C") return "Chase Paymentech Orbital - Tampa";
            if (bankcardService.ServiceId == "8A4B91300C") return "Chase Paymentech Orbital - Salem";
            if (bankcardService.ServiceId == "19F161300C") return "First Data - Nashville";
            if (bankcardService.ServiceId == "3257B1300C") return "Chase Paymentech Orbital - Tampa Retail";
            if (bankcardService.ServiceId == "859AC1300C") return "First Data - Nashville US";
            if (bankcardService.ServiceId == "633511300C") return "Chase Tampa Direct TCS";
            if (bankcardService.ServiceId == "355931300C") return "RBS Global Gateway";
            if (bankcardService.ServiceId == "8CEA11300C") return "RBS Worldpay";
            if (bankcardService.ServiceId == "168511300C") return "TSYS Sierra";
            if (bankcardService.ServiceId == "852BB1300C") return "Tampa - Canada";
            if (bankcardService.ServiceId == "507BF1300C") return "TSYS Sierra Canada";
            if (bankcardService.ServiceId == "55C3C1300C") return "TSYS Summit";
            if (bankcardService.ServiceId == "D1DDF1300C") return "Salem Direct";
            if (bankcardService.ServiceId == "D917B1300C") return "First Data - Nashville";
            if (bankcardService.ServiceId == "7AC431300C") return "Intuit QBMS";
            if (bankcardService.ServiceId == "7B4DD1300C") return "First Data BUYPASS";
            if (bankcardService.ServiceId == "9461F1300C") return "Tampa TCS for Canada";
            if (bankcardService.ServiceId == "CE4AE1300C") return "Intuit QBMS Inline Tokenization";
            if (bankcardService.ServiceId == "E7DFB1300C") return "Intuit QBMS No Tokenization";
            if (bankcardService.ServiceId == "CAFF61300C") return "Adaptive Payments";
			if (bankcardService.ServiceId == "9999999999") return "Vantiv FTPS";//NOTE : Placeholder

            return bankcardService.ServiceName;//Match was not found so pass back the name in the service.
        }
        ...
        public TypeCardType CardTypeLookup(string strPAN)
        {
            ...
            */
            ...
            if (Convert.ToInt16(strPAN.Substring(0, 1)) == 36)
                return TypeCardType.MasterCard;//MC reissued Diners

            if (Convert.ToInt16(strPAN.Substring(0, 2)) == 30 | Convert.ToInt16(strPAN.Substring(0, 2)) == 38)
                return TypeCardType.MasterCard;//MC/Diners co-branded

            if (Convert.ToInt16(strPAN.Substring(0, 4)) == 6011)
                return TypeCardType.Discover;

            if (Convert.ToInt16(strPAN.Substring(0, 3)) > 644 & Convert.ToInt16(strPAN.Substring(0, 3)) < 659)
                return TypeCardType.Discover;

            if (Convert.ToInt16(strPAN.Substring(0, 3)) > 300 & Convert.ToInt16(strPAN.Substring(0, 3)) < 305)
            ...
            return TypeCardType.NotSet; //No match was found
        ...
        {
            //Sandbox
            if (storedValueService.ServiceId == "C58FD00001") return "Vantiv Stored Value Service";

            //Production
            if (storedValueService.ServiceId == "XXXXX1300C") return "Vantiv Stored Value Service";
