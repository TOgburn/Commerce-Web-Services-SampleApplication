...
using CustomerPresent=schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.CustomerPresent;
using TypeISOCountryCodeA3=schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.TypeISOCountryCodeA3;
using TypeStateProvince=schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.TypeStateProvince;
using AccountType = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.AccountType;
using AVSData = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.AVSData;
using BankcardInterchangeData = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardInterchangeData;
using BankcardTenderData = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardTenderData;
using BankcardTransaction = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardTransaction;
using BankcardTransactionData = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardTransactionData;
using BankcardTransactionDataPro = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BankcardTransactionDataPro;
...
using BillPayment = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.BillPayment;
using CardData = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.CardData;
using CardSecurityData = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.CardSecurityData;
using CommercialCardResponse = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.CommercialCardResponse;
using CustomerInfo = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.CustomerInfo;
using CVDataProvided = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.CVDataProvided;
using ExistingDebt = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.ExistingDebt;
using GoodsType = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.GoodsType;
using IsTaxExempt = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.IsTaxExempt;
using ItemizedTax = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.ItemizedTax;
using Level2Data = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.Level2Data;
using LineItemDetail = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.LineItemDetail;
using NameInfo = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.NameInfo;
using RequestCommercialCard = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.RequestCommercialCard;
using Response = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.Response;
using Tax = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.Tax;
using TaxExempt = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.TaxExempt;
using TransactionCustomerData = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.TransactionCustomerData;
using TypeCardType = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.TypeCardType;
using TypeUnitOfMeasure = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.TypeUnitOfMeasure;
using TypeISOCurrencyCodeA3 = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.TypeISOCurrencyCodeA3;
using EntryMode = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.EntryMode;
using IndustryType = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.IndustryType;
using PartialApprovalSupportType = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.PartialApprovalSupportType;
...
namespace SampleCode
{
    ...
    #region Variable Declarations
        ...
        private ServiceInformation _si;
        private List<string> MerchantProfileIds = new List<string>();
        ...
        private bool _blnEncryptedIdentityToken;//Flag used to determine if the identity token is encrypted or not. Recommendation is to alway encrypt.
        ...
        public SampleCode_DeskTop()
        {
            ...
            txtDisclaimer.Text = "Copyright (c) 2004-2010 IP Commerce, INC. - All Rights Reserved.\r\n\r\n"
                                 + " This software and documentation is subject to and made"
                                 + " available only pursuant to the terms of an executed license"
                                 + " agreement, and may be used only in accordance with the terms"
                                 + " of said agreement. This software may not, in whole or in part,"
    ...
    #region Preparing to Transact
        ...
        {   //The SignOn() operation facilitates the authentication of identity tokens and the exchange of session tokens during application 
            ...
            if (chkDedicated.Checked & Helper.ServiceKey.Length < 1)
            ...
            if (chkDynamic.Checked & txtIdentityToken.Text.Length < 1)
            ...
            catch (EndpointNotFoundException)
            {
                ...
                try
                {
                    Helper.SetSvcEndpoint();//Change the endpoint to use the backup.
                    Helper.SessionToken = Helper.Cwssic.SignOnWithToken(txtIdentityToken.Text);
                    Helper.DtSessionToken = DateTime.UtcNow;
                    chkStep1.Checked = true;
                }
        ...
        private void cmdAddNewProfile_Click(object sender, EventArgs e)
        {
        ...
        {
          if (cboAvailableProfiles.SelectedIndex == -1) { MessageBox.Show("Please select a Profile"); return; }
    ...
    #region Transaction Processing
            ...
            else if (_svas != null) //Process a Stored Value Transaction
            {
                ...
                try
                {
                    ...
                    if (_svas.Tenders.CreditAuthorizeSupport == CreditAuthorizeSupportType.AuthorizeAndCaptureOnly)
                    {
                        ...
                        //Now in ordert to process a ReturnById we must first Capture the transaction. This can be done with CaptureAll or CaptureSelective.
                        response = Helper.ProcessBCPTransaction(TransactionType.CaptureAll, null, null, null, null, null, null, null, null, ChkAcknowledge.Checked, false);
                        if (response.Count < 1) { return; }
                        ChkLstTransactionsProcessed.Items.Add(response[0]);
        ...
        {//The Return() operation is used to return funds to a payment account without associating the return with a specific transaction that has been previously captured.
            ...
            if (_bcs != null) //Process a BankCard Transaction
            {
                ...
                {
                    BankcardTransaction BCtransaction = SetBankCardTxnData();
                    processResponse(Helper.ProcessBCPTransaction(TransactionType.Return, BCtransaction, null, null, null, null, null, null, null, ChkAcknowledge.Checked, false));
                }
                catch (Exception ex) { MessageBox.Show(ex.Message); }
                finally { Cursor = Cursors.Default; }
            }
        ...
        {//The QueryAccount() operation is used to perform a balance inquiry on a cardholder's account to determine the current account balance.
            ...
            try
            {
                ElectronicCheckingTransaction ECKTransaction = SetElectronicCheckTxnData();
                //Let's Query a transaction
                processResponse(Helper.ProcessECKTransaction(TransactionType.QueryAccount, ECKTransaction, null, null, null, ChkAcknowledge.Checked));
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
            finally { Cursor = Cursors.Default; }
            }
        ...
        private void cmdVerify_Click(object sender, EventArgs e)
        {
            ...
            if (_bcs != null) //Process a BankCard Transaction
            {
                try
                {
                    ...
                    processResponse(Helper.ProcessBCPTransaction(TransactionType.Verify, BCtransaction, null, null, null, null, null, null, null, ChkAcknowledge.Checked, false));
                }
                catch (Exception ex) { MessageBox.Show(ex.Message); }
                finally { Cursor = Cursors.Default; }
            }
         ...
         // The following will use an Authorize follwed by the RequestTransaction to simulate a dropped response.
            ...
            if (_bcs != null) //Process a BankCard Transaction
            {
                try
                {
                    ...
                    Helper.ProcessBCPTransaction(TransactionType.Authorize, BCtransaction, null, null, null, null, null, null, null, ChkAcknowledge.Checked, ChkUserWorkflowId.Checked);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
        ...
        private void cmdCardTokenization_Click(object sender, EventArgs e)
        {
            ...
            if (_bcs != null) //Process a BankCard Transaction
            {
                try
                {
                    ...
                    BCtransaction = SetBankCardTxnData();//Reset the back card information
            ...
            if (_bcs != null) //Process a BankCard Transaction
            {
                try
                {
                    ...
                    {//This is a host capture service so use the AuthorizeAndCapture transaction
                        processResponse(Helper.ProcessBCPTransaction(TransactionType.AuthorizeAndCapture, BCtransaction, null, null, null, null, null, null, null, ChkAcknowledge.Checked, ChkUserWorkflowId.Checked));
                    }
                }
                catch (Exception ex) { MessageBox.Show(ex.Message); }
                finally { Cursor = Cursors.Default; }
        ...
        #endregion Service Information Helpers
        ...
        private StoredValueTransaction SetStoredValueTxnData()
        {
        ...
        private static Level2Data SetLevel2Data()
        {
        ...
        private static LineItemDetail SetLevel3Data()
        {
            LineItemDetail LID = new LineItemDetail();
        ...
        #endregion END Set Transaction Data
        ...
        {
            if (txtServiceKey.Text.Length < 1 & chkDedicated.Checked) { MessageBox.Show("Please enter a valid Service Key"); return; }
            ...
            if (chkDedicated.Checked)
            {
                ...
                txtSvcPrimary.Text = Helper.BaseSvcEndpointPrimary + "/" + Helper.ServiceKey;
                txtSvcSecondary.Text = Helper.BaseSvcEndpointSecondary + "/" + Helper.ServiceKey;
                txtTxnPrimary.Text = Helper.BaseTxnEndpointPrimary + "/" + Helper.ServiceKey;
                txtTxnSecondary.Text = Helper.BaseTxnEndpointSecondary + "/" + Helper.ServiceKey;
            }
            ...
            if (Helper.ServiceKey.Length > 0 && LoadPersistedConfig())
            {
        ...
        private void cmdPopulateWithMyTestValues_Click(object sender, EventArgs e)
        {
            ...
            txtServiceKey.Text = "";
            if (txtServiceKey.Text.Length < 1 & txtIdentityToken.Text.Length > 1)
                txtServiceKey.Text = Helper.RetrieveServiceKeyFromIdentityToken(txtIdentityToken.Text.Trim()); 

            if (chkDedicated.Checked)
            {
                if (txtServiceKey.Text.Length < 1) { MessageBox.Show("Dedicated Solution requires a Service Key"); return; }
                //Set the Primary and Secondary Endpoints
                txtSvcPrimary.Text = Helper.BaseSvcEndpointPrimary + "/" + txtServiceKey.Text;
                txtSvcSecondary.Text = Helper.BaseSvcEndpointSecondary + "/" + txtServiceKey.Text;
                txtTxnPrimary.Text = Helper.BaseTxnEndpointPrimary + "/" + txtServiceKey.Text;
                txtTxnSecondary.Text = Helper.BaseTxnEndpointSecondary + "/" + txtServiceKey.Text;
            }
            if (chkDynamic.Checked)
            {
                //Set the Primary and Secondary Endpoints
        ...
        {
            if (chkDedicated.Checked && !_checkChangedDirty)
            {
            ...
            else if (!chkDedicated.Checked && !_checkChangedDirty)
            {
        ...
        {
            if (chkDynamic.Checked && !_checkChangedDirty)
            {
                _checkChangedDirty = true;
            ...
            else if (!chkDynamic.Checked && !_checkChangedDirty)
            {
        ...
        private void lnkServiceKey_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
