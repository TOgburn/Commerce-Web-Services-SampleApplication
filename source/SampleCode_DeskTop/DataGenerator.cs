...
{//Please note that the DataGenerator class will not likely be used for a software company's solution. Instead the information below would come from a database
    class DataGenerator
    {
        ...
        public static IndustryTypeValues _ITV = getIndustryType(ConfigurationSettings.AppSettings["IndustryType"]);
        private static SampleCode_DeskTop target;
        #endregion Variable Declarations

        public DataGenerator(SampleCode_DeskTop _myForm)
        {
            target = _myForm;
        }
        ...
        public static BankcardTransactionPro SetBankCardTxnData(BankCardProProcessingOptions _BCPPO)
        {
            ...
            TxnData.Amount = Convert.ToDecimal( (target.TxtAmount.Text));
            ...
            if (target.TxtTip.Text.Length > 0)
                if (Convert.ToDecimal(target.TxtTip.Text) > 0)
                    TxnData.TipAmount = Convert.ToDecimal(target.TxtTip.Text);

            if (target.chkProcessAsPINDebitTxn.Checked)//Cashback for PINDebit only
                if (target.TxtCashBack.Text.Length > 0)
                    if (Convert.ToDecimal(target.TxtCashBack.Text) > 0)
                        TxnData.CashBackAmount = Convert.ToDecimal(target.TxtCashBack.Text);
            ...
            if (target.ChkAllowPartialApprovals.Checked)
            ...
            if (target.ChkCardNotPresent.Checked)
            {//Keyed Transaction
                TxnData.EntryMode = EntryMode.Keyed;
                BCtransaction.TenderData.CardData.CardType = (TypeCardType)target.CboCardTypes.SelectedItem;
                BCtransaction.TenderData.CardData.Expire = target.TxtExpirationDate.Text; // Note : that in a swipe track data the format is "YYMM" however here it's "MMYY"
                BCtransaction.TenderData.CardData.PAN = target.TxtPAN.Text;
                BCtransaction.TenderData.CardData.CardholderName = "Dirk Pit";
            }
            else if (target.ChkTokenization.Checked)
            {//Card tokenization
                try
                {
                    TxnData.EntryMode = EntryMode.Keyed;
                    TokenizedTransaction T = (TokenizedTransaction)target.CboTokenizedCard.SelectedItem;
                    BCtransaction.TenderData.CardData = new CardData();
            ...
            else if (target.ChkTrackDataFromMSR.Checked)
            {//Swiped Transaction
                ...
                BCtransaction.TenderData.CardData.CardType = (TypeCardType) target.CboCardTypes.SelectedItem;
                string SwipeToSeperateTracks = target.TxtTrackDataFromMSR.Text;
            ...
            if (blnAVS | blnCVV | target.chkProcessAsPINDebitTxn.Checked)
            {
                ...
                if (blnCVV)
                {
                    ...
                    if ((TypeCardType)target.CboCardTypes.SelectedItem == TypeCardType.AmericanExpress)
                ...
                if (target.chkProcessAsPINDebitTxn.Checked)
                {
            ...
            if (target.ChkProcessAsPINLessDebit.Checked)
            {
        ...
        public static ElectronicCheckingTransaction SetElectronicCheckTxnData()
        {
            ...
            //Transaction Data
            ECKTransaction.TransactionData.Amount = Convert.ToDecimal(target.TxtAmount.Text);
        ...
        public static StoredValueTransaction SetStoredValueTxnData()
        {
            ...
            //Transaction Data
            SVATransaction.TransactionData.Amount = Convert.ToDecimal(target.TxtAmount.Text);
        ...
        public static IndustryTypeValues getIndustryType(string _industryType)
        {
            ...
            if (_industryType == "")
            {
                ...
                i._ApplicationLocation = (schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.ApplicationLocation)Enum.Parse(typeof(schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.ApplicationLocation), ConfigurationSettings.AppSettings["ApplicationLocation"]);
                i._PINCapability = (schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.PINCapability)Enum.Parse(typeof(schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.PINCapability), ConfigurationSettings.AppSettings["PINCapability"]);
                i._ReadCapability = (schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.ReadCapability)Enum.Parse(typeof(schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.ReadCapability), ConfigurationSettings.AppSettings["ReadCapability"]);
                i._EncryptionType = (EncryptionType)Enum.Parse(typeof(EncryptionType), ConfigurationSettings.AppSettings["EncryptionType"]);

                //MerchantData
                i._IndustryType = (schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.IndustryType)Enum.Parse(typeof(schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.IndustryType), ConfigurationSettings.AppSettings["IndustryType"]);
                i._CustomerPresent = (schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.CustomerPresent)Enum.Parse(typeof(schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.CustomerPresent), ConfigurationSettings.AppSettings["CustomerPresent"]);
                i._RequestACI = (schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.RequestACI)Enum.Parse(typeof(schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.RequestACI), ConfigurationSettings.AppSettings["RequestACI"]);
                i._EntryMode = (schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.EntryMode)Enum.Parse(typeof(schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.SvcInfo.EntryMode), ConfigurationSettings.AppSettings["EntryMode"]);
            ...
            try
            {
                ...
                try
                {
                    if (File.Exists(target.Helper.ServiceKey + "_TransactionProcessing.config"))
                    {
                        ...
                        if (target.Helper.ApplicationProfileId.Length > 0) target.chkStep2.Checked = true;
                        target.txtPersistedAndCached.Text = "ApplicationProfileId : " + target.Helper.ApplicationProfileId + "\r\nServiceId : " + target.Helper.ServiceID + "\r\nWorkflowId : " + target.Helper.WorkflowID + "\r\nMerchantProfileId : " + target.Helper.MerchantProfileId + strEncryptedIdentityToken;
                    }
                    return true;
                }
                catch (Exception Ex)
        ...
        {
            if (target.CboCardTypes.Text == "Visa")
            {
                target.TxtPAN.Text = "4111111111111111";
                target.TxtTrackDataFromMSR.Text = "%B4111111111111111^IPCOMMERCE/TESTCARD^13121010454500415000010?;4111111111111111=13121010454541500010?";
            }
            else if (target.CboCardTypes.Text == "MasterCard")
            {
                target.TxtPAN.Text = "5454545454545454";
                target.TxtTrackDataFromMSR.Text = "%B5454545454545454^IPCOMMERCE/TESTCARD^1312101013490000000001000880000?;5454545454545454=13121010134988000010?";
            }
            else if (target.CboCardTypes.Text == "AmericanExpress")
            {
                target.TxtPAN.Text = "371449635398456";
                target.TxtTrackDataFromMSR.Text = "%B371449635398456^IPCOMMERCE/TESTCARD^1312060523319?;371449635398456=1312060523319?";
            }
            else if (target.CboCardTypes.Text == "Discover")
            {
                target.TxtPAN.Text = "6011000995504101";
                target.TxtTrackDataFromMSR.Text = "%B6011000995504101^IPCOMMERCE/TESTCARD^13121011000627210201?;6011000995504101=13121011000627210201?";
            }
            else if (target.CboCardTypes.Text == "PrivateLabel")
            {
                target.TxtPAN.Text = "8818889876543211";
                target.TxtTrackDataFromMSR.Text = ";8818889876543211=13121014764094900001?";
            }
            else
            {
                target.TxtPAN.Text = "";
                target.TxtTrackDataFromMSR.Text = "";
            }
        }
    }
    public class Item
